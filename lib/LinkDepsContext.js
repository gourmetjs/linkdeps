...
proto.parseArgv = function(argv) {
  ...
  options.srcPath = argv._[0];
  options.desPath = argv.out;

  if (argv.deploy)
    options.mode = "deploy";
  else if (argv["deploy-mix"])
    options.mode = "deploy-mix";
...
proto.run = function() {
  ...
  return wrap(function() {
    ...
    if ((self.mode === "devel" || self.mode === "link") && !self.check)
