...
proto.addAllLocalsAsFolderPath = function(privateOnly) {
  this.forEachLocal(function(local, ref) {
    if (privateOnly && ref.isRoot && !local.pkg.private)
      return;
...
proto.forEachLocal = function(callback) {
  this._buildTree();
  this._traverseTree(callback, this._tree);
};

proto._traverseTree = function(callback, ref) {
  var self = this;
  ref.locals.forEach(function(local) {
    self._traverseTree(callback, local);
  ...
  function _diff(section) {
    ...
    if (diff && diff.length) {
      ...
      diff.forEach(function(item) {
        ...
        if (refs)
          refs = "(" + refs + ")";
        output.push(nutil.format("%s %s: %s %s", item.action, item.name, item.semver, refs));
      });
    }
  }
