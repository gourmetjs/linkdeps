...
function LinkDepsContext(options) {
  ...
  this._desPath = options.desPath ? npath.resolve(process.cwd(), options.desPath) : this._srcPath;
  this._outDeps = {};
      ...
      if (bins) {
        ...
        return Promise.all(bins.map(function(item) {
          ...
          if (process.platform === "win32") {
            ...
            return createLink(symlink, target);
          }
        }));
      }
    }

    var moduleDir = npath.resolve(self._desPath, "node_modules");
    var bins = _parseBin();
...
proto.saveResult = function(space, basePkg) {
  var res = this.getResult();
  var pkg = clone(basePkg || this._srcPackage);
  ...
  var content = JSON.stringify(pkg, null, space);

  mkdirp.sync(this._desPath);
  fs.writeFileSync(npath.join(this._desPath, "package.json"), content, "utf8");
};
...
proto.getDesRelPath = function(path) {
  path = npath.relative(this._desPath, path);
  return _slashPath(path);
};
